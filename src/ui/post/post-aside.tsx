...
export function StatusHistory({ history }: { history: StatusChangeView[] }) {
  return (
      ...
      <div className="flex flex-wrap items-center gap-1.5">
        ...
        {rest > 0 && (
          <span className="tnum text-small text-muted">
            и ещё {formatCount(rest)}
          </span>
        )}
      </div>
      <p className="mt-2 tnum text-small text-faint">
        всего {formatCount(total)} {plural(total, countLabel)}
      </p>
    </Section>
  )
}
...
export function MergedPosts({
  ...
  return (
    <Section title="Объединённые обращения">
        ...
        {merged.map((item) => (
          <li key={item.ref}>
            ...
            <p className="mt-0.5 tnum text-small text-faint">
              {item.ref} · {formatCount(item.movedVotes)}{' '}
              {plural(item.movedVotes, ['голос', 'голоса', 'голосов'])} перенесено
            </p>
          </li>
        ))}
      </ul>
