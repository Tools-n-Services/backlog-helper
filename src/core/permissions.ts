...
export type Permission =
  ...
  | 'post.merge'
  /** Публиковать запись changelog: закрывает обращения и рассылает (FR-165). */
  | 'release.publish'
...
const REQUIRED: Record<Permission, AccessRole> = {
  ...
  'post.merge': 'admin',
  /* Публикация релиза рассылает письма всем, кто голосовал за связанные
     обращения, и отозвать их нельзя — необратимость та же, что у объединения,
     а адресатов больше. */
  'release.publish': 'admin',
  'user.ban': 'admin',
