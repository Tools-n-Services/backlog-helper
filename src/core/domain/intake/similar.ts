 ...
 *
 * Повторяет поведение `pg_trgm`: сам поиск выполняет Postgres запросом
 * `word_similarity(title, :q) >= :threshold`, а здесь живут порог и правила
 * нормализации — они обязаны совпадать с тем, что делает база.
 *
...
/**
 * Порог триграммного совпадения для поиска по ленте (FR-115).
 *
 * Выше, чем у врезки похожих: там человек ещё ничего не искал и лишний
 * кандидат его не раздражает, а здесь он сформулировал запрос и ждёт по нему
 * ответа. Но ниже стандартных 0.6 в Postgres — на них не находится даже
 * «экпорт» вместо «экспорт», а опечатка в поиске это норма, а не исключение.
 */
export const SEARCH_TRIGRAM_THRESHOLD = 0.45
