...
/**
 * Пометить снимок устаревшим.
 *
 * Вызывается из мутаций справочников: человек, который только что переименовал
 * статус, обязан увидеть новое имя на следующем экране, а не через время жизни
 * снимка. Остальные процессы догонят сами.
 *
 * ИМЕННО ПОМЕТИТЬ, А НЕ ОБНУЛИТЬ. Обнуление ломало чужие запросы: страница,
 * которая уже начала рисоваться и читает справочник синхронно в мапперах,
 * посреди рендера получала «справочники не загружены» — то есть админ,
 * сохраняющий настройки, ронял страницу постороннему читателю. Устаревший
 * снимок хуже свежего ровно на одно чтение и не ломает ничего.
 */
