...
| `pnpm worker` | Фоновые проходы: рассылка, пересчёты, сверка (`--loop` — постоянно) |
| `pnpm worker` | Разослать письма из очереди (`--loop` — постоянно) |
...
рассылка подписчикам через очередь.

Не сделано: вложения к обращениям (таблица есть, объектное хранилище не подключено),
бэклог и синхронизация с трекером (таблицы есть, экранов нет), нагрузочная проверка
на 15 000 обращений.
