...
**English:** [README.md](README.md) · **Документация:** [русская](docs/) · [English](docs/en/) · [Español](docs/es/)

**Портал обратной связи, который вы разворачиваете у себя.** Пользователи присылают
запросы и сообщения об ошибках, команда их разбирает, а продуктовый бэклог, роадмап
и changelog собираются из этого же потока.

Ставится на свой сервер одной командой и настраивается в браузере — без правки кода.
Один образ обслуживает сколько угодно продуктов: у каждого свой домен, своя база
и свои настройки.
...
От слоя зависит, чем и когда правится настройка.

| Слой | Что там | Как меняется |
|---|---|---|
| **Админка** | название, знак, домен, язык, разделы, лимиты, сроки, фирменный цвет, доски, статусы, типы обращений и поля их форм | на живом портале, сразу |
| **Среда (`.env`)** | адрес базы, канал почты и ключи, хранилище, переводчик, токен установки | перезапуском контейнера |
| **Код (`config/*.ts`)** | виды полей, смысл системных полей, формула приоритета, палитра, правила вложений | пересборкой образа |
| **Никогда** | `slug` доски, `key` статуса и типа после создания | — ломает ссылки и историю |

Подробно — [docs/10-operate.md](docs/10-operate.md).
